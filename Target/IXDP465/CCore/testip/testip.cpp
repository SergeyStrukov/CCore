...
IPEngine::FindInfo::FindInfo(Dev::PCIBus::HostInitExit &pci_host)
 {
  info=pci_host.find(Driver::VIA_Rhine_III::VendorId,Driver::VIA_Rhine_III::DeviceId);
...
IPEngine::IPEngine()
 : info(pci_host),
   eth_device(info,pci_host.getAddressMap()),
   eth_master(eth_device,"Eth"),
   ip_device("Eth",Net::IPAddress(192,168,1,10),Net::IPAddress(255,255,255,0)),
   ip_master(ip_device,"IP"),
   start_stop(eth_device,TaskPriority(5000))
 {
 }
